...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Basic Handling for dummy register
    $name = $_POST['name'];
    $email = $_POST['email'];
    $password = $_POST['password'];

    // This is where you would database-authenticate your user with $pdo
    // Example: $stmt = $pdo->prepare("INSERT INTO users (name, email, password) VALUES (?, ?, ?)");
...
<head>
    ...
    <style>
        ...
        button { background-color: #28a745; color: white; border: none; padding: 0.75rem; border-radius: 4px; width: 100%; cursor: pointer; font-size: 1rem; }
